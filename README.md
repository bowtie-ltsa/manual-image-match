...
We assume that the number of image pairs is large compared to the number of volunteers. (e.g. 100 images would yield roughly ~4500-5000 image pairs [depending on number of captures in in folder]. For ~10 volunteers, that would be ~460-500 decisions per volunteer.) This has implications for how an intelligent human coordinator would handle volunteers asking for `opportunities` to make decisions:
...
- Toward the end of round `k`, when there are only a few image pairs left to be `decided`, we will end up with more volunteers than we have image pairs left in the round. That is okay. We choose to have those volunteers `swarm` the remaining images (that is, we choose to have multiple volunteers given the same opportunity), because this helps ensure that the round will finish as quickly as possible (or at all).
- A consequence of `swarming` is that it is possible that a few image pairs will get some "extra" decisions (i.e., when round `k` finishes, all image pairs will have `k` decisions, and a few might have more than `k` decisions.) How many is "a few"? At the end of round 1, no more than `v/2` image pairs. Fewer if there happen to be few volunteers active at the end of the round, or their timing is good. For later rounds, it gets harder to calculate and we haven't bothered since we assume that "extra" images are not really a concern. For `k` far less than `N` (the number of image pairs), we would guess an upper bound of `k*(v/2)`, although this number can be reduced by omitting image pairs from the next round that have already met the goal of that round. Note that it is possible but perhaps unlikely that the very last image pair in a round could get many decisions at the last minute (up to `v` of them).
- We also feel that an intelligent coordinator would never tell a willing volunteer to wait for the next round; that it is better to have the volunteer swarm, or even start the next round if necessary, than turn them away.
...
- Navigate forward and backward in the decision history (previous, next, start, end, -10, +10).
...
- **The Opportunity Board**: When a volunteer is given an `Opportunity`, it is removed from `The Opportunity List` (so it is not given to other volunteers), and added to `The Opportunity Board` (so it can be tracked). 
  - Near the end of a round (i.e. when `the opportunity list` is empty), several volunteers can "swarm" on the last remaining opportunities (i.e. get added to opportunities from `the opportunity board`).
  - The opportunity stays on `the opportunity board` until someone makes a decision; it is removed as soon as the first volunteer makes a decision.
...
- **Bucket Board**: A BucketBoard is a "board" that can only have up to one opportunity on it. Each volunteer has their own `bucket board`.
  ...
  - This will add a decision beyond the goal of the current round, but that's better than telling the volunteer to wait for the round to end (which might be a while). 
...
8. Check to see if there is a `swarming` opportunity:
   ...
   - Among those that are valid for the volunteer, consider only those with the least number of volunteers already (to spread the volunteers out over more image pairs, when swarming is happening).
...
9. If there is anything at all on the volunteer's `bucket list` (there should be, cf step #4 above):
   1. Add that opportunity to the volunteers `bucket board`.
   2. Provide that opportunity (and stop here).
